...
export const sendNotifications = async (): Promise<any[]> => {
  ...
  const notifications = await prisma.notification.findMany({ where: { emailStatus: EmailStatus.PENDING_RETRY }, include: { member: true } })
  ...
  return Promise.all(
    notifications.map(async (notification) => {
      ...
      try {
        ...
        await prisma.notification.update({ where: { id }, data: { emailStatus: EmailStatus.SENT, isSent: true, retryCount: 0 } })
        return
      } catch (errData) {
        error('Email notification failure', `Failed to email ${id} with error:`, errorMessage(errData))
        if (retryCount <MAX_RETRY_COUNT) {
          await prisma.notification.update({ where: { id }, data: { emailStatus: EmailStatus.PENDING_RETRY, retryCount: retryCount + 1 } })
          return
        }
        await prisma.notification.update({ where: { id }, data: { emailStatus: EmailStatus.FAILED_TO_SEND, retryCount: 0 } })
      }
    })
  )
}
