...
export const generateCategories = (depth: number, parent?: RawForumCategoryMock) => (mocks: MocksForForum): RawForumCategoryMock[] => {
...
const generateCategory = (depth: number, parent?: RawForumCategoryMock) => (mocks: MocksForForum): RawForumCategoryMock[] => {
  ...
  const category: RawForumCategoryMock = {
    ...
    moderatorIds: faker.random.arrayElements(mocks.workers, randomFromRange(1, 8)).map((worker) => worker?.id as string),
    status: {
