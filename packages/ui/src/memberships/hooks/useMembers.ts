...
import { useGetMembersCountQuery, useGetAllMembersQuery } from '@/memberships/queries'
...
export const useMembers = ({ order, filter, perPage = 10 }: UseMemberProps) => {
  ...
  const { data, loading, error: err } = useGetAllMembersQuery({ variables })
  ...
  return {
    isLoading: loading,
    members: data?.memberships.map(asMemeberWithDetails) ?? [],
    totalCount,
    pagination,
  }
}
