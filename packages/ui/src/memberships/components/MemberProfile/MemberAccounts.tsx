...
interface BoundAccountProps {
  account: string
}

const BoundAccount = ({ account }: BoundAccountProps) => {
  const boundBalance = useBalance(account)

  return (
    <AccountMemberRow key={account}>
      <UnknownAccountInfo address={account} placeholderName="Bound Account" />
      <TokenValue value={boundBalance?.total} isLoading={!isDefined(boundBalance?.total)} />
    </AccountMemberRow>
  )
}
