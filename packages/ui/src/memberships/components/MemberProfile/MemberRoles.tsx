...
export const MemberSideRoles = ({ member }: { member: Member }) => {
  return (
    <RolesDisplay gap={8}>
      {member.roles ? <MemberSideRole member={member} /> : <TextMedium light>This profile has no roles.</TextMedium>}
    </RolesDisplay>
  )
}

const MemberSideRole = ({ member }: { member: Member }) => {
  return (
    <MemberRoleToggle absoluteToggle>
      {(isOpen) => {
        return (
...
const MemberRoleToggle = styled(ToggleableItem)`
  position: relative;
