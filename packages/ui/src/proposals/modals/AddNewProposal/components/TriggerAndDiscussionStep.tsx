...
export const TriggerAndDiscussionStep = () => {
	...
	const { selected, setSelected } = useState(false); // add input
			...
			<RowGapBlock gap={20}>
				<InlineToggleWrap>
					...
					<ToggleCheckbox
						falseLabel="Open"
						trueLabel="Closed"
						name="triggerAndDiscussion.isDiscussionClosed"
					/>
					<Tooltip
				...
				{isDiscussionClosed && (
					<RowGapBlock gap={20}>
						<TextMedium lighter>
							Closed mode: only the active council, the original proposer, or one among a set of
							whitelisted members can post.
						</TextMedium>
						<InputComponent
						...
						>
							<SelectMember
								...
								filter={(member) =>
									!discussionWhitelist.find(
										(whitelistMember: Member) => whitelistMember.id === member.id
									)}
							/>
							{selected ? (
						...
						<WhitelistContainer>
							{discussionWhitelist.map((member: Member) => (
								<WhitelistMember key={member.id}>
									...
									<WhitelistRemoveMember
										onClick={() => removeMemberFromWhitelist(member)}
										id="removeMember"
									/>
								</WhitelistMember>
							))}
