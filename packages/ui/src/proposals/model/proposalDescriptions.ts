...
export const proposalDescriptions: ProposalDescriptions = {
  ...
  createWorkingGroupLeadOpening: 'Same effect as when creating an opening for workers in the given group with given inputs, except the opening type is for lead.',
  fillWorkingGroupLeadOpening: 'Same effect as when filling opening in group for worker with given inputs.',
  updateWorkingGroupBudget: 'Positive budget update gets debited from the council budget and credited to the group budget, otherwise the reverse.',
  ...
  terminateWorkingGroupLead: 'Same as when terminating a worker in group with given inputs, and removing lead designation.',
  ...
  setCouncilBudgetIncrement: 'This proposal sets how much tokens is added to council budget every new budget period. This budget is spent on working groups, spending proposals and council rewards.',
  setCouncilorReward: 'All councilors are paid out the same flat reward rate from the councillor budget, subject to budget constraints. ',
  setInitialInvitationBalance: 'Invitation balance is deposited to new member’s account to cover basic transactions. This balance can only be used on fees.',
  setInitialInvitationCount: 'New members have default allocation of new members invite. Invites can be transferred to other members.',
  setMembershipLeadInvitationQuota: 'Membership Workgroup Lead is automatically assigned invitations upon taking the role. This proposals is aimed on managing this invitations count.',
  setReferralCut: 'Referrals, same as new membership invitations are incentivised by the platform. Referral cut entails the reward to the originator of invitation links which resulted in new memberships.',
  updateChannelPayouts: 'Proposal to submit the Channel Incentives Payout payload, update the min/max claimable amounts and block/allow claiming of awarded JOYs by channels.',
